...
public class IDSrendServlet extends HttpServlet
{
	...
	/** 組宋體用的工具 */
        /*e The Tool for compositing Song font .*/
	protected IDSrendService 宋體組字工具;
        /** 組宋體粗體用的工具 */
        /*e The Tool for compositing bold Song font . */
	protected IDSrendService 粗宋組字工具;
        /** 組楷體用的工具 */
        /*e The Tool for compositing Kai font .*/
	protected IDSrendService 楷體組字工具;
        /** 組楷體粗體用的工具 */
        /*e The Tool for compositing Kai bold font .*/
	protected IDSrendService 粗楷組字工具;

	/** 產生圖形傳予組字介面畫。毋過無X11、圖形介面就袂使用 */
	//GraphicsConfiguration 系統圖畫設定;
	/** 佮資料庫的連線 */
	protected PgsqlConnection 連線; //e 連線 means connetcion


	/** 組字出來的字型解析度 */
	int 字型大細;  //e 字型大細 means font resolution
	...
	public IDSrendServlet()
	{
		...
		MergePieceAdjuster 調整工具 = new MergePieceAdjuster( //e 調整工具 means tuning tool.
		...
		MkeSeparateMovableType_Bolder 活字加粗 = new MkeSeparateMovableType_Bolder( //e 活字加粗 means making a separate movable type bolder.
		...
		*/
		...
		ChineseCharacterTypeSetter 楷體設定工具 = new FontRefSettingTool(展開式查通用字型編號工具, FontCorrespondTable
				...
				java.awt.RenderingHints.VALUE_TEXT_ANTIALIAS_DEFAULT,
				java.awt.RenderingHints.VALUE_FRACTIONALMETRICS_DEFAULT), 活字加粗);
		/*e 楷體設定工具 means Kai font setting tool.
		   others are similar to SongFontSettingTool
		 */

		楷體組字工具 = new IDSrendService(查詢方式, 正規化工具, 楷體設定工具, 調整工具, 活字加粗, 普通字型屬性, 字型大細);
		//e 楷體組字工具 means Kai font compositetool, others similar to  SongFontCompositeTool.
		...
		ChineseCharacterTypeSetter 粗楷設定工具 = new FontRefSettingTool(展開式查通用字型編號工具, FontCorrespondTable
				...
				java.awt.RenderingHints.VALUE_TEXT_ANTIALIAS_DEFAULT,
				java.awt.RenderingHints.VALUE_FRACTIONALMETRICS_DEFAULT), 活字加粗);
		//e 粗楷設定工具 means bold Kai font  setting tool. others are similar to bold Song font setting tool.

		粗楷組字工具 = new IDSrendService(查詢方式, 正規化工具, 粗楷設定工具, 調整工具, 活字加粗, 粗字型屬性, 字型大細);
		//e 粗楷組字工具 means bold Kai font compositetool, others similar to  SongFontCompositeTool.
	}
			...
			HttpServletResponse response) throws IOException
	{
		...
		if (是舊網址(網址字串))// TODO ==字體 ,  // "是舊網址" means isOldURL. 
		{
			...
			String 新網址 = String.format("/%s?%s=%s",
						      URLEncoder.encode(目錄[1], "UTF-8"), //e 新網址 means new URL,目錄 means folder.
						      URLEncoder.encode("字體", "UTF-8"), //e 字體 means font.
						      URLEncoder.encode(目錄[0], "UTF-8")); //e 字體 means font
		...
		if (附檔名.equals("svg"))
		{
			...
			組字工具.字組成svg(組字式, response.getOutputStream()); //e 組字工具 means  composite font tool, 字組成svg means glyph composited svg,組字式=IDS
		}
    ...
    private boolean 是舊網址(String 網址字串)  //e 是舊網址 means isOldUrl , 網址字串 means url string.
	{
	    String[] 目錄 = 網址字串.split("/"); //e 目錄 means folder

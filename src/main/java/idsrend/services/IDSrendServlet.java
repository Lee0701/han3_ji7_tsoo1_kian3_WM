...
public class IDSrendServlet extends HttpServlet
{
	...
	protected IDSrendService 宋體組字工具;
        /** 組宋體粗體用的工具 */
        /* The Tool for compositing bold Song font . */
	protected IDSrendService 粗宋組字工具;
        /** 組楷體用的工具 */
        /* The Tool for compositing Kai font .*/
	protected IDSrendService 楷體組字工具;
        /** 組楷體粗體用的工具 */
        /* The Tool for compositing Kai bold font .*/
	...
	public IDSrendServlet()
	{
		...
		IDSnormalizer 正規化工具 = new IDSnormalizer(); // 正規化工具mean  IDS normalizer
		...
		int 粗字型屬性 = Font.BOLD; // 粗字型屬性 means bold font type property
		int 普通字型屬性 = 0; // 普通字型屬性 means normal font type property
		字型大細 = 200; // 字型大細 means font resolution
		MkeSeparateMovableType_Bolder 活字加粗 = new MkeSeparateMovableType_Bolder( // 活字加粗 means making a sparate movable type bolder.
			...
			HttpServletResponse response) throws IOException
	{
		...
		if (是舊網址(網址字串))// TODO ==字體 ,  // "是舊網址" means isOldURL. 
		{
			...
			String 新網址 = String.format("/%s?%s=%s",
						      URLEncoder.encode(目錄[1], "UTF-8"), // 新網址 means new URL,目錄 means folder.
    ...
    private boolean 是舊網址(String 網址字串)  // 是舊網址 means isOldUrl , 網址字串 means url string.
	{
	    String[] 目錄 = 網址字串.split("/"); // 目錄 means folder
